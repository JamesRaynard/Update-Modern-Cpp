...
template <typename T> void func(const T& arg) {
	cout << "ArgType: ";
	...
	if constexpr(is_const_v<arg_no_ref>) {
		cout << "const ";
	}
...
int main() {
	...
	cout << "func(x): ";
	func(x);                        // ArgType = const int&, T = int
	cout << "\nfunc(cx): ";
	func(cx);                       // ArgType = const int&, T = int
	cout << "\nfunc(rcx): ";
	func(rcx);                      // ArgType = const int&, T = int
